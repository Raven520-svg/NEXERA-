...
.account-box {border: 2px dashed #FFD700; padding: 15px; border-radius: 10px; background-color: #1a1a1a;}
...
def init_db():
    ...
    # Settings table for voting time
    c.execute('''CREATE TABLE IF NOT EXISTS settings
                 (key TEXT PRIMARY KEY, value TEXT)''')
    # Insert default settings if not exist
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('voting_active', '0')")
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('voting_start', '')")
    c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('voting_end', '')")
    conn.commit()
    conn.close()
...
CATEGORIES = ["Music", "Dance", "Comedy", "Content Creator", "Fashion Design", "Food & Catering", "Beauty & Makeup", "Barbing", "Tech", "Art", "Crafts", "Farming", "Sports", "Other"]
...
def show_countdown():
    ...
    if start and end:
        ...
        if now < end_time:
            ...
            st.info(f"⏰ Voting Ends In: {str(remaining).split('.')[0]}")
        else:
...
with menu[0]:
    ...
    else: st.info("No contestants yet.")
        ...
        if not df.empty:
                ...
                for j in range(2):
                    if i + j < len(df):
                        ...
                        with cols[j]:
                            ...
                            st.write(f"### {row['name']}"); st.write(f"**{row['talent']} - {row['state']}**")
                            st.write(f"**Reason:** {row['reason']}"); st.write(f"**VERIFIED VOTES:** {row['votes']}")
                            if st.button("VOTE", key=f"vote_btn_{row['id']}"):
            ...
            if 'voting_for' in st.session_state:
                ...
                with st.form("vote_form"):
                    ...
                    st.markdown(f"""<div class="account-box"><h4>Pay ₦{VOTE_PRICE} to:</h4><p><b>Bank:</b> {VOTING_ACCOUNT['Bank']}<br><b>Account Name:</b> {VOTING_ACCOUNT['Account Name']}<br><b>Account No:</b> {VOTING_ACCOUNT['Account No']}</p></div>""", unsafe_allow_html=True)
...
with menu[2]:
    ...
    with st.form("submission_form", clear_on_submit=True):
        ...
        location = st.text_input("City/Location *"); bank = st.text_input("Bank Account Details *")
        reason = st.text_area("Why do you need NEXERA capital? *", height=150)
        ...
        if st.form_submit_button("SUBMIT NOW"):
            if name and phone and talent!= "Select..." and state!= "Select..." and location and bank and reason and photo:
                ...
                st.success("✅ Submission received! Awaiting approval.")
...
with menu[3]:
    ...
    st.write("How it works: Talented people submit. The community votes with ₦200. 100% of vote money goes directly to contestants. Top 3 winners get ₦120k, ₦70k, ₦30k.")
...
with menu[4]:
    ...
    if password == ADMIN_PASSWORD:
        ...
        if st.button("TURN ON VOTING" if not voting_active else "TURN OFF VOTING"):
            ...
            if new_status == '1': set_setting('voting_start', str(datetime.now()))
            if new_status == '0': set_setting('voting_end', str(datetime.now() + timedelta(days=7))) # 7 days voting
            st.rerun()
        ...
        for i, row in df_sub.iterrows():
            if st.button(f"Approve {row['name']}", key=f"approve_{row['id']}"):
        ...
        for i, row in df_votes.iterrows():
            st.write(f"**{row['voter_name']}** voted for **{row['contestant_name']}**")
            if row['proof'] and os.path.exists(row['proof']): st.image(row['proof'], width=200)
...
# --- FOOTER ---
st.markdown("---"); st.write(f"**Support:** {SUPPORT_EMAIL} | {SUPPORT_WHATSAPP}")
